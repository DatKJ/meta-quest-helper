...
涉及驱动下载地址、ADB 版本或最新要求时，优先打开官方链接核实。开发者模式开启步骤统一读 `usage-tips.md`。

如果用户需要使用 ADB，但当前电脑或运行环境没有 `adb` 命令、无法识别 `adb`，或 `adb version` 无法正常输出，先说明需要安装 Android SDK Platform Tools。下载、解压、配置 PATH 或安装相关工具前，必须询问用户并取得明确同意；未经同意不要自动下载或安装任何工具。取得同意后，agent 可以为用户自动安装 ADB，但必须优先使用官方 Android Platform Tools 下载页，并在执行前说明安装位置、是否修改 PATH、以及如何卸载或删除。
