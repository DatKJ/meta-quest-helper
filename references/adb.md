...
先确认用户目标是合法开发、调试、安装自有/开源/授权 APK，还是传输个人文件。不要帮助安装盗版、破解商店应用、规避授权或付费限制、卸载系统核心组件、root、刷机或运行来路不明脚本。
...
- Android ADB 官方文档：搜索关键词：`Android ADB official documentation`
- Android SDK Platform Tools 下载页：搜索关键词：`Android SDK Platform Tools official download`
- Windows Oculus ADB Drivers：从 Meta 设备设置文档中的 `Oculus ADB Drivers` 链接进入；链接可能随 Meta 文档更新而变化。

涉及驱动、ADB 版本或最新要求时，优先使用上述搜索关键词找到官方资料核实。开发者模式开启步骤统一读 `usage-tips.md`。

如果用户需要使用 ADB，但当前电脑或运行环境没有 `adb` 命令、无法识别 `adb`，或 `adb version` 无法正常输出，先说明需要安装 Android SDK Platform Tools。下载、解压、配置 PATH 或安装相关工具前，必须询问用户并取得明确同意；未经同意不要自动下载或安装任何工具。取得同意后，agent 可以为用户自动安装 ADB，但必须优先使用官方 Android SDK Platform Tools 页面，并在执行前说明安装位置、是否修改 PATH、以及如何卸载或删除。
...
1. 搜索关键词：`Android SDK Platform Tools official download`，从官方页面进入。
...
适合不想手动管理命令行、需要设备管理、日志、性能工具的用户。它不能代替对 ADB 基础的理解；遇到命令行问题仍可回到 Platform Tools 排查。
...
先提醒用户：这只能修复“联网检测误判”。如果 Wi-Fi 本身不可用、DNS 错误、路由器断网、连接配置没有生效或 Meta 服务不可达，这些命令不会让网络凭空可用。

### 方式 A：改成更容易访问的联网检测地址

```text
adb shell settings put global captive_portal_http_url <可信的 204 检测 HTTP 地址>
adb shell settings put global captive_portal_https_url <可信的 204 检测 HTTPS 地址>
adb shell settings put global captive_portal_fallback_url <可信的 204 检测备用地址>
adb reboot
```

执行前提醒用户先通过搜索确认可信的 204 检测地址，不要随意复制不明来源地址。重启后重新连接 Wi-Fi，观察“受限”提示是否消失。如果用户不想重启，也可以先关闭再打开 Wi-Fi，但重启更干净。
...
不要把这组命令描述成“解锁 Meta 服务”的方案；它只处理 Android 联网检测/受限提示。
...
不要指导用户安装盗版、破解或规避授权限制的 APK。遇到这类请求时，改为推荐官方商店、App Lab、开源项目、用户自建 APK 或开发调试流程。
